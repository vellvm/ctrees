...
Inductive all_finite {E C} {A: Type} : ctree E C A -> Prop :=
 | all_finiteRet: forall a t,
    observe t = RetF a ->
   all_finite t
 | all_finiteBr: forall {X} (c: C X) b t k,
   observe t = BrF b c k ->
 ...
 | all_finiteVis: forall {X} (e: E X) t k,
   ...
   (forall x, all_finite (k x)) ->
   all_finite t.
#[global] Hint Constructors all_finite : ctree.

(** Smart constructors *)
(* all_finite *)
Lemma all_finite_Ret : forall E C R a,
  @all_finite E C R (Ret a).
Proof.
  intros; econstructor 1; reflexivity.
Qed.
#[global] Hint Resolve all_finite_Ret : ctree.

Lemma all_finite_Br : forall E C X R b (c: C X) k,
  (forall x, all_finite (k x)) ->
  @all_finite E C R (Br b c k).
Proof.
  intros; econstructor 2; [reflexivity | eauto].
Qed.
#[global] Hint Resolve all_finite_Br : ctree.

Lemma all_finite_Vis : forall E C X R (e : E X) k,
  (forall x, all_finite (k x)) ->
  @all_finite E C R (Vis e k).
Proof.
  intros; econstructor 3; [reflexivity | eauto].
Qed.
#[global] Hint Resolve all_finite_Vis : ctree.

(** Inversion lemmas *)

(* all_finite *)
Lemma all_finite_Tau_inv : forall E C X R (c: C X) b k,
  @all_finite E C R (Br b c k) ->
  forall x, all_finite (k x).
Proof.
...
Lemma all_finite_Vis_inv : forall E C X R (e : E X) k,
  @all_finite E C R (Vis e k) ->
  forall x, all_finite (k x).
Proof.
...
#[global] Instance all_finite_equ {E C A}:
  Proper (equ eq ==> iff) (@all_finite E C A).
Proof.
  repeat intro; split; intro.
  - revert y H; induction H0; intros u EQ;
    step in EQ; cbn in EQ; rewrite H in EQ; inv EQ; try invert.
    + eapply all_finiteRet; eauto. 
    + eapply all_finiteBr; eauto.
    + eapply all_finiteVis; eauto.
  - revert x H; induction H0; intros u EQ; symmetry in EQ;
      step in EQ; cbn in EQ; rewrite H in EQ; inv EQ; try invert.
    + eapply all_finiteRet; eauto. 
    + eapply all_finiteBr; eauto.
...
Lemma all_finite_bind : forall {E C R S}
  (t : ctree E C R) (k : R -> ctree E C S),
  all_finite t ->
  (forall x, x ∈ t -> all_finite (k x)) ->
  all_finite (t >>= k).
Proof.
  intros * FIN; induction FIN; intros FINs;
    rewrite unfold_bind, H; eauto with ctree.
Qed.

Lemma all_finite_bind_weak : forall {E C R S}
  (t : ctree E C R) (k : R -> ctree E C S),
  all_finite t ->
  (forall x, all_finite (k x)) ->
  all_finite (t >>= k).
...
Lemma all_finite_bind_inv : forall {E C R S}
  (t : ctree E C R) (k : R -> ctree E C S),
  all_finite (t >>= k) ->
  (all_finite t /\ forall x, x ∈ t -> all_finite (k x)).
Proof.
  intros E C R S.
  cut (forall (u : ctree E C S), all_finite u ->
       forall (t : ctree E C R) k, u ≅ t >>= k ->
        (all_finite t /\ forall x, x ∈ t -> all_finite (k x))).
  intros LEM * FIN; eapply LEM; eauto; reflexivity.
  induction 1; intros * EQ.
  - rewrite (ctree_eta t), H in EQ; clear H t.
Admitted.
    (*

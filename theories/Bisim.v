...
(* 
  ...
  | SchedFork {n} (x : Fin.t n) k t : schedule_ (observe (k x)) t -> schedule_ (ForkF k) t
  | SchedRet x : schedule_ (RetF x) (RetF x)
  | SchedVis {X} (e : E X) k : schedule_ (VisF e k) (VisF e k).

  Definition schedule u v := schedule_ (observe u) (observe v).
 *)

(* 
  Inductive step : ctree E R -> option {X : Type & E X} -> ctree E R -> Prop := 
  | StepFork {n} (x : Fin.t n) k obs t : step (k x) obs t -> step (Fork k) obs t
  | StepVis {X} (e : E X) k x : step (Vis e k) (Some (existT _ _ e)) (k x)
  | StepTau t obs t' : step t obs t' -> step (Tau t) obs t'
  | StepRet x : step (Ret x) None (Ret x). *)
...
Section bisim.
  ...
  Variant matching (bisim : itree E R1 -> itree E R2 -> Prop) : 
  ...
  Variant bisimF (bisim : itree E R1 -> itree E R2 -> Prop) :
    itree E R1 -> itree E R2 -> Prop :=
  ...
  (* 
     Finite and productive trees are handled by [BisimSched]: it is a old style bisimulation built atop of a notion of
     stepping in the tree by walking accross internal non-deterministic nodes.
      ...
      Fork 2 (Ret 0) (Ret 0) ~ Ret 0 false
      Fork 2 spin spin ~ spin     true

    However this does not cover silent divergence, it gives us eutt without the Tau/Tau rule.
    To recover this, and generalize it, we use [BisimDiv]: we can match forks, assuming that all branches are bisimilar.
    Note that this allow us to prove [Fork1^ω ~ Fork2^ω] but also that [Fork2 (Fork1^ω) (Ret 0) ~ Ret 0]

    We should also have some commutation and rebalancing properties: [Fork2 (Fork2 u v) w ~ Fork3 u v w]
  *)
...
(* begin hide *)
Global Hint Constructors eq_itreeF: core.
Global Hint Constructors bisimF: core.
Global Hint Resolve eq_itreeF_mono : paco.
Global Hint Constructors matching: core.

Lemma schedule_vis_inv : 
  forall {E X Y} e (k : X -> itree E Y) t, 
    schedule (Vis e k) t -> t ≡ Vis e k.
Proof.
  intros * SCHED; inv SCHED; dependent destruction H1; dependent destruction H2; auto.
Qed.

Ltac inv_eq H := punfold H; inv H.

Lemma equ_vis_invT {E X Y S} (e1 : E X) (e2 : E Y) (k1 : X -> itree E S) k2 :
  Vis e1 k1 ≡ Vis e2 k2 -> 
  X = Y.
Proof.
  intros EQ; punfold EQ; dependent destruction EQ; reflexivity.
Qed.

Lemma equ_vis_invE {E X S} (e1 e2 : E X) (k1 k2 : X -> itree E S) :
  Vis e1 k1 ≡ Vis e2 k2 -> 
  e1 = e2 /\ forall x, k1 x ≡ k2 x.
Proof.
  intros EQ; punfold EQ; dependent destruction EQ; pclearbot; auto.
Qed.

Instance eq_itree_schedule {E X}: Proper (paco2 (@eq_itreeF E X X eq) bot2 ==> paco2 (eq_itreeF eq) bot2 ==> iff) schedule.
Proof.
  ...
  - punfold EQ1; punfold EQ2. 
	...
	Lemma foo :
	forall {E X Y} {RR: X -> Y -> Prop},
	 subrelationH (@eq_itree E _ _ RR) (@bisim E _ _ RR).
	 intros; red.
 ...
 Global Hint Unfold bisim_: core.
 Global Hint Resolve bisim__mono : paco.
 Global Hint Resolve bisim_idclo_mono : paco.
 ...
 Proof.
	 ...
	 destruct (observe s); cbn; gstep; constructor; eauto with paco.
 Qed.
 ...
 Proof.
	 ...
	 1: apply reflexivity.
	 all: gstep; constructor; eauto with paco.
 Qed.

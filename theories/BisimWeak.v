...
Section wbisim.
  ...
  | WBisim u t
            ...
            (SIMFE : forall u', schedule u u' ->
              exists t', schedule t t' /\ matching' wbisim u' t')
            (SIMBE : forall t', schedule t t' ->
              exists u', schedule u u' /\ matching' wbisim u' t')
               :
      wbisimF wbisim u t.
...
(* 
  ...
  Lemma matching'_mono u v sim sim'
              (IN : matching' sim u v)
              (LE : sim <= sim') :
        matching' sim' u v.
  Proof.
    ...
    apply LE; auto.
  Qed.
  Hint Resolve matching'_mono: core.

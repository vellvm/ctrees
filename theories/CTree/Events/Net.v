  ...
  Equations schedule_one' (R: fin' n -> sys n T -> ctree logE void)
    ...
    schedule_one' R r system with observe (prog (system $ r)) => {
          ...
          | Some obs =>          
              Vis (Log obs) (fun _: unit =>
                               R to (system
                                       @ r do (uprog (k tt))
                                                @ to do (umail (Some m))))
          | None => R to (system @ r do (uprog (k tt)) @ to do (umail (Some m)))
          end;
        ...
        schedule_one' _ _ _ (VisF Recv k) :=
          ...
          | Some obs =>          
              Vis (Log obs) (fun _: unit => R r (system @ r do (fun t => umail None (uprog (k m) t))))
          | None => R r (system @ r do (fun t => umail None (uprog (k m) t)))
          end;
      }.    

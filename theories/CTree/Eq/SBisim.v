(*|
=========================================
Strong Bisimulation over ctrees
=========================================
...
|*)
  ...
  #[global] Instance equ_ss_closed_goal {r} : Proper (@equ E HE X X eq ==> @equ F HF Y Y eq ==> flip impl) (ss L r).
  Proof.
  ...
  #[global] Instance equ_ss_closed_ctx {r} : Proper (@equ E HE X X eq ==> @equ F HF Y Y eq ==> impl) (ss L r).
  Proof.
  ...
  Program Definition sbisim_clos {LE LF} : mon (rel (ctree E X) (ctree F Y)) :=
...
Section Proof_Rules.
  ...
  Lemma step_sb_brD_gen {n m} (k : fin' n -> ctree E X) (k' : fin' m -> ctree F Y) (R: rel _ _) :
    (forall x, exists y, sb L R (k x) (k' y)) ->
    (forall y, exists x, sb L R (k x) (k' y)) ->
    sb L R (BrD n k) (BrD m k').
  Proof.
    intros EQs1 EQs2.
    split; apply step_ss_brD_gen; intros.
    - destruct (EQs1 x) as [z [FW _]]; now (exists z). 
    - destruct (EQs2 x) as [z [_ BA]]; now (exists z).
  Qed.

  Lemma step_sb_brD {n m} (k: fin' n -> ctree E X) (k': fin' m -> ctree F Y) (R: rel _ _) :
    (forall x, exists y, sbt L R (k x) (k' y)) ->
    (forall y, exists x, sbt L R (k x) (k' y)) ->
    sbt L R (BrD n k) (BrD m k').
  Proof. now apply step_sb_brD_gen. Qed.

  Lemma step_sb_brD_id_gen {n} (k: fin' n -> ctree E X) (k': fin' n -> ctree F Y) (R: rel _ _) :
    (forall x, sb L R (k x) (k' x)) ->
    sb L R (BrD n k) (BrD n k').
  Proof.
  ...
  Lemma step_sb_brD_id {n} (k : fin' n -> ctree E X) (k': fin' n -> ctree F Y) (R: rel _ _) :
    (forall x, sbt L R (k x) (k' x)) ->
    sbt L R (BrD n k) (BrD n k').
  Proof. now apply step_sb_brD_id_gen. Qed.
...
Section Sb_Proof_System.
 ...
 Lemma sb_guard_l: forall (t u : ctree E X),
  ...
  Lemma sb_brD_inv_l: forall n (t: ctree E X)
                        (k: fin' n -> ctree E X),
      BrD n k ~ t ->
      forall i, k i ~ t.
  Proof.
    ...
    step in H.
    inv H.
    eapply ss_brD_l_inv with (x:=i) in H0.
  ...
  Lemma sb_brS {n m} (k : fin' n -> ctree E X) (k': fin' m -> ctree E X) :
  ...
  Lemma sb_brS_id {n} (k k' : fin' n -> ctree E X) :
    (forall x, k x ~ k' x) ->
    BrS n k ~ BrS n k'.
  Proof.
    intros * EQs; step.
  ...
  Lemma sb_brD {n m} (k : fin' n -> ctree E X) (k': fin' m -> ctree E X) :
    (forall x, exists y, k x ~ k' y) ->
    (forall y, exists x, k x ~ k' y) ->
    BrD n k ~ BrD m k'.
  Proof.
  ...
  Lemma sb_brD_id {n} (k k': fin' n -> ctree E X):
    (forall x, k x ~ k' x) ->
    BrD n k ~ BrD n k'.
  Proof.
    intros; step.
  ...
  Lemma sb_unfold_forever: forall (k: X -> ctree E X)(i: X),
      forever k i ~ r <- k i ;; forever k r.
  Proof.
    intros k i.
    rewrite (ctree_eta (forever k i)).
    unfold forever, Classes.iter, MonadIter_ctree.    
    cbn.
    rewrite <- ctree_eta.
    rewrite unfold_iter, bind_map.
...
|*)
  ...
  Lemma brD2_assoc : forall (t u v : ctree E X),
      brD2 (brD2 t u) v ~ brD2 t (brD2 u v).
  Proof.
    intros.
    play.
    - trans in TR.
      + trans in TR; exists l, t'; auto with trans.
      + exists l, t'; auto with trans.
    - trans in TR.
      + exists l, t'; auto with trans.
      + trans in TR; exists l, t'; auto with trans.
  Qed.

  Lemma brD2_commut: forall (t u : ctree E X),
      brD2 t u ~ brD2 u t.
  Proof.
    intros.    
    play; trans in TR; exists l, t'; auto with trans.
  Qed.

  Lemma brD2_idem : forall (t : ctree E X),
      brD2 t t ~ t.
  Proof.
    intros.
    play.
  ...
  Lemma brD2_merge : forall (t u v : ctree E X),
      brD2 (brD2 t u) v ~ brD3 t u v.
  Proof.
    intros.
    play.
    trans in TR.
    - trans in TR; exists l, t'; split; auto.
      * now apply trans_brD with t Fin.F1.
      * now apply trans_brD with u (Fin.FS Fin.F1).
    - exists l, t'; split; auto.
      now apply trans_brD with v (Fin.FS (Fin.FS Fin.F1)).
    - trans in TR; exists l, t'; split; auto; etrans.
  Qed.

  Lemma brD2_is_stuck : forall (u v : ctree E X),
      is_stuck u ->
      brD2 u v ~ v.
  Proof.
    intros * ST.
    ...
    - trans in TR.
      + exfalso.
        now apply ST; exists l, t'.
      + etrans.
    - exists l, t'; split; etrans.
  Qed.

  Lemma brD2_stuck_l: forall (t : ctree E X),
      brD2 stuck t ~ t.
  Proof.
    intros; apply brD2_is_stuck, stuck_is_stuck.
  Qed.

  Lemma brD2_stuck_r: forall (t : ctree E X),
      brD2 t stuck ~ t.
  Proof.
    intros; rewrite brD2_commut, brD2_stuck_l; reflexivity.
  Qed.

  (*| BrS2 is commutative and "almost" idempotent |*)
  Lemma brS2_commut : forall (t u : ctree E X),
      brS2 t u ~ brS2 u t.
  Proof.
    intros.
    play; trans in TR.
    - exists tau, t; split; [etrans |]; split; [now rewrite Eqt | reflexivity].
    - exists tau, u; split; [etrans |]; split; [now rewrite Eqt | reflexivity].
    - exists tau, u; split; [etrans |]; split; [now rewrite <- Eqt | reflexivity].
    - exists tau, t; split; [etrans |]; split; [now rewrite <- Eqt | reflexivity].
  Qed.

  Lemma brS2_idem : forall X (t : ctree E X),
      brS2 t t ~ step t.
  Proof.
    intros.
    play; trans in TR; setoid_rewrite Eqt; exists tau, t; etrans.
  Qed.
  ...
  Lemma sbisim_BrS_inv {Y n m} (k1 : fin' n -> ctree E X) (k2: fin' m -> ctree E Y) :
    BrS n k1 ~ BrS m k2 ->
  ...
  Lemma sbisim_ret_BrS_inv {n} (r: X) (k : fin' n -> ctree E X) :
    Ret r ~ BrS n k -> False.
  Proof.
    intros * abs.
    eplayL.
  ...
  Lemma sbisim_vis_BrS_inv{n} (e: E) (k1 : encode e -> ctree E X) (k2: fin' n -> ctree E X):
  ...
  Lemma sbisim_ret_BrD_inv {n} (r: X) (k : fin' n -> ctree E X) :
    Ret r ~ BrD n k ->
    exists x, Ret r ~ k x.
  Proof.
    ...
    - repeat intro. inv H0. exists (val r), x0. split; intuition. rewrite <- Hs.
      rewrite ctree_eta. rewrite <- H3, <- ctree_eta, <- H4. reflexivity.
...
Section StrongSimulations.
  ...
  Lemma sbisim_clos_ss : @sbisim_clos E HE F HF X Y eq eq <= (sst L).
  Proof.

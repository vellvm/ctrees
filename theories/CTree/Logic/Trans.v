...
Section CTreeTrans.
  ...
  Variant ktrans_: relation (ctree E X * World E) :=    
    ...
    | KtransTau (w: World E) (t t': ctree E X):
      ...
      not_done w ->
      ktrans_ (t, w) (t', w)
    | KtransDone (x: X) (t t': ctree E X) w:
      trans (val x) t stuck ->
      t' ≅ stuck ->
      not_done w ->
      ktrans_ (t, w) (t', Done x).
  ...
  Proof.
    ...
    - inv H. 
      ...
      + exists (val x); rewrite H5; split; auto; split; auto with ctl.
        right; right.
        exists x; auto.
    - destruct H as (l & ? & ? & []).
      + destruct H1 as (-> & ->).
        now apply KtransTau.
      + destruct H1 as [(e & x & -> & ->) | (x & -> & ? & ->)].
        * now apply KtransObs.
        * rewrite H1 in *.
...
Section CTreeTrans.
  ...
  Lemma ktrans_ret {X}: forall (t: ctree E X) (w w': World E) (e: E) (v: encode e) x,
      ktrans_ (Ret x, w) (t, w') <-> (not_done w /\ w' = Done x /\ t ≅ stuck).
  Proof.
    intros; split; intros.
    - ktrans_inv H; dependent destruction H; split; auto.
  ...
  Lemma ktrans_pure{X}: forall (t t': ctree E X) w,
      ktrans_ (t, w) (t', Pure) -> w = Pure.
  Proof.
    intros * H.
    inv H; reflexivity.
  Qed.
  Hint Resolve ktrans_pure: ctl.
  ...
  Proof.
    intros.
    ktrans_inv H; apply ktrans_trans; try world_inv.
    - inv H1. 
    - ktrans_inv H0. 
      + exists tau; split.
  ...
  Lemma ktrans_bind_inv{X Y}: forall (t: ctree E Y) (u: ctree E X) (k: Y -> ctree E X) (w w': @World E HE) ,
  ...
  Proof.
    ...
    - right; exists y; split; auto; apply ktrans_trans.
...
Proof.    
  ...
  - apply ktrans_vis in TR as (y & ? & ? & ?); world_inv.
Qed.

  ...
  Proof.
    ...
    + dependent destruction i; try solve [inv i];
      ...
      * edestruct IHn with (k:= fun i => k (Fin.FS i)) (i:=@Fin.F1 n); eauto.
        ...
        -- apply TR.
        -- 
      destruct H; dependent destruction x; try solve [inv x].
      split; auto.
      intros.
      now apply H0 with Fin.F1.
    + next; split.
...
Section SyntacticLemmas.
  ...
  Lemma axdone_ind: forall t w,
      <( t, w |= AX done φ )> -> AXDoneInd (observe t) w.
  Proof.
    intros.
    ...
    - destruct b.
      ...
      + constructor.
        intros i.
        apply ax_brD_inv with (i:=i) in H.
      apply ax_brD_inv in H.

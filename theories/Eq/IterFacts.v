...
Definition iter_gen {E X I} (step : I -> ctree E (I + X)) (t : ctree E (I + X)) :=
  r <- t;;
  match r with
...
Lemma iter_iter_gen {E X I} : forall (step : I -> ctree E (I + X)) (i : I),
...
Inductive trans_it {E X I} (step : I -> ctree E (I + X)) (t : ctree E (I + X)) l T : Prop :=
| it_next i :
...
Lemma trans_it_brD {E X I} : forall (step : I -> ctree E (I + X)) t l c k x T,
  t ≅ BrD c k ->
...
Lemma iter_gen_brS {E X I} : forall (step : I -> ctree E (I + X)) t c K,
...
Proof.
  ...
  - rewrite bind_br in H.
    ...
    simple eapply equ_br_invE with (x := x) in H. apply H.
Qed.

Lemma iter_gen_vis {E X I Y} : forall (step : I -> ctree E (I + X)) t (e : E Y) K,
...
Theorem trans_iter_gen : forall {E X I} l (step : I -> ctree E (I + X)) t T, trans l (iter_gen step t) T -> trans_it step t l T.
Proof.
  ...
  - destruct H0.
    + unfold iter_gen in H0.
      ...
      * destruct H1. rewrite H1, bind_ret_l in H0. destruct x0.
        ...
        eapply it_next.
        --rewrite H1. etrans.
        --apply IHtrans_; auto. setoid_rewrite iter_iter_gen in H0.
          apply equ_br_invT in H0 as ?.
          destruct H2 as [? _]. inversion H2. subst. clear H2.
          simple apply equ_br_invE with (x := x) in H0.
          left. rewrite <- H0, <- ctree_eta. reflexivity.
      * destruct H1 as (? & ? & ?).
    ...
    + apply equ_br_invT in H0 as ?. destruct H1 as [-> _].
      simple apply equ_br_invE with (x := x) in H0.
      apply IHtrans_. left. rewrite <- ctree_eta. apply H0. reflexivity.
...
#[global] Instance sbisim_iter_gen {E A B} :
  @Proper ((A -> ctree E (A + B)) -> ctree E (A + B) -> ctree E B)
    (pointwise_relation _ sbisim ==> sbisim ==> sbisim)
    iter_gen.
Proof.
  repeat intro.
  ...
  - step in H2. apply H2 in H0 as [].
    apply trans_val_inv in H0 as ?. rewrite H4 in H0. clear x H3 H4.
    apply IHtrans_it in H as []. clear IHtrans_it.
    exists x; auto.
    unfold iter_gen.
    apply trans_bind_r with (x := inl i); auto.
    apply trans_guard. apply H.
  - destruct H0 as (? & ? & ?).
    step in H1. apply H1 in H0 as [].
    exists (iter_gen step' x).
    + apply trans_bind_l; auto.
    + rewrite H2. apply CH; auto.
  - subst.
    step in H3. apply H3 in H0 as [].
    apply trans_val_inv in H0 as ?. rewrite H4 in H0. clear x H1 H4.
    exists stuckD.
    eapply trans_bind_r. apply H0. cbn. etrans. now rewrite H2.
Qed.

#[global] Instance sbisim_iter {E A B} :
  @Proper ((A -> ctree E (A + B)) -> A -> ctree E B)
    (pointwise_relation _ sbisim ==> pointwise_relation _ sbisim)
    iter.
Proof.
  repeat intro.

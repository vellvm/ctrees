...
Section equg.
  ...
  Context {E B : Type -> Type} {R1 R2 : Type}.
  Context `{HasB1 : B1 -< B}.
  Context (RR : R1 -> R2 -> Prop).

  Inductive equgb (eq : ctree E B R1 -> ctree E B R2 -> Prop) :
  ...
  | Eqg_GuardL t1 t2 k1
      ...
      equgb eq (brDF branch1 k1) t2
  | Eqg_GuardR t1 t2 k2
              (Hk : forall x, k2 x = t2)
              (REL : equgb eq t1 (observe t2)) :
      equgb eq t1 (brDF branch1 k2)
  | Eqg_Br b {X} (c : B X) (k1 : X -> _) (k2 : X -> _)
              (REL : forall i, eq (k1 i) (k2 i)) :
      equgb eq (BrF b c k1) (BrF b c k2)
  .
  Hint Constructors equgb: core.

  Definition equgb_ eq : ctree E B R1 -> ctree E B R2 -> Prop :=
  ...
  Program Definition fequg: mon (ctree E B R1 -> ctree E B R2 -> Prop) := {|body := equgb_|}.
  Next Obligation.
  ...
  Theorem equgb_fequg : forall (R : rel _ _) (t : ctree E B R1) (t' : ctree E B R2), equgb R (observe t) (observe t') = fequg R t t'.
  Proof.
    reflexivity.
  Qed.
...
Definition equg {E B R1 R2} `{B1 -< B} R := (gfp (@fequg E B R1 R2 _ R)).
...
Ltac fold_equg :=
  repeat
    match goal with
    | h: context[@fequg ?E ?B ?R1 ?R2 _ ?RR] |- _ => fold (@equg E B R1 R2 _ RR) in h
    | |- context[@fequg ?E ?B ?R1 ?R2 _ ?RR] => fold (@equg E B R1 R2 _ RR)
    end.
...
Tactic Notation "__step_equg" :=
  match goal with
  | |- context [@equg ?E ?B ?R1 ?R2 _ ?RR _ _] =>
      unfold equg;
      step;
      fold (@equg E B R1 R2 _ RR);
      simpl body
  | |- _ => step
  end.
...
Section equg_theory.
  ...
  Notation gT  := (coinduction.T (fequg (E := E) (B := B) RR)).
  Notation gt  := (coinduction.t (fequg (E := E) (B := B) RR)).
  Notation gbt := (coinduction.bt (fequg (E := E) (B := B) RR)).
(*|
...
|*)
  Definition seq: relation (ctree E B R) := eq.
	...
	Proof.
		...
		change (@eq (ctree E B R)  <= equgb_ RR eq).
...
|*)
  ...
  #[global] Instance Reflexive_equgb (equg : ctree E B R -> ctree E B R -> Prop) :
...
#[global] Hint Constructors equgb : core.
Arguments equgb_ {E B R1 R2} _ RR eq t1 t2/.

#[global] Instance Reflexive_equg {E B X} `{B1 -< B}: Reflexive (@equg E B X X _ eq).
Proof.
  apply Reflexive_gt. typeclasses eauto.
Qed.

#[global] Instance Symmetric_equg {E B X} `{B1 -< B}: Symmetric (@equg E B X X _ eq).
Proof.
  apply Symmetric_gt. typeclasses eauto.
Qed.

Theorem equ_clos_gt {E B X} `{HasB1 : B1 -< B} : @equ_clos E E B B X X <= gt eq.
Proof.
  ...
  - step in Equt. dependent induction Equt.
    cbn. rewrite <- x. econstructor. intros. destruct x0. reflexivity.
    apply IHHR; auto. rewrite <- (Hk tt), <- ctree_eta, REL. reflexivity.
  - step in Equu. dependent induction Equu.
    cbn. rewrite <- x. econstructor. intros. destruct x0. reflexivity.
    apply IHHR; auto. rewrite <- (Hk tt), <- ctree_eta, REL. reflexivity.
  - step in Equt. dependent induction Equt.
...
|*)
Lemma equg_vis_invT {E B X Y S} `{B1 -< B} (e1 : E X) (e2 : E Y) (k1 : X -> ctree E B S) k2 :
...
Lemma equg_vis_invE {E B X S} `{B1 -< B} (e1 e2 : E X) (k1 k2 : X -> ctree E B S) :
  Vis e1 k1 (G≅) Vis e2 k2 ->
  e1 = e2 /\ forall x, k1 x (G≅) k2 x.
Proof.
  intros EQ; step in EQ.
	inv EQ.
	dependent destruction H2. invert.
	auto.
Qed.

(*Lemma equg_br_invT {E B S b b' X1 X2} `{B1 -< B} (c1 : B X1) (c2 : B X2) (k1 : _ -> ctree E B S) k2 :
  Br b c1 k1 (G≅) Br b' c2 k2 ->
  n <> 1 -> m <> 1 ->
  n = m /\ b = b'.
Proof.
  intros EQ; step in EQ.
...
Lemma equgb_vis_invT {E B X Y S} `{B1 -< B} (e1 : E X) (e2 : E Y) (k1 : X -> ctree E B S) k2 :
...
Lemma equgb_vis_invE {E B X S} `{B1 -< B} (e1 e2 : E X) (k1 k2 : X -> ctree E B S) :
  equb eq (equg eq) (VisF e1 k1) (VisF e2 k2) ->
...
#[global] Instance equg_observe {E B R} `{HasB1 : B1 -< B} :
  Proper (equg eq ==> going (equg eq)) (@observe E B R).
Proof.
  constructor. step in H.
  now step.
Qed.

#[global] Instance equg_BrF {E B R X} `{B1 -< B} b (c : B X) :
  Proper (pointwise_relation _ (equg eq) ==> going (equg eq)) (@BrF E B R _  b _ c).
Proof.
  constructor. red in H. step. econstructor; eauto.
Qed.

#[global] Instance equg_VisF {E B R X} `{B1 -< B} (e : E X) :
  Proper (pointwise_relation _ (equg eq) ==> going (equg eq)) (@VisF E B R _ _ e).
Proof.
  constructor. red in H. step. econstructor; eauto.
Qed.

#[global] Instance observing_sub_equg E B R `(HasB1 : B1 -< B) :
  subrelation (@observing E B R R eq) (equg eq).
Proof.
  repeat intro.
  step. rewrite (observing_observe H). apply Reflexive_equgb; eauto.
Qed.

#[global] Instance equ_clos_gt_goal E B X RR `(HasB1 : B1 -< B) :
  Proper (@equ E B X X eq ==> equ eq ==> flip impl) (gt eq RR).
Proof.
  cbn; intros ? ? eq1 ? ? eq2 H.
  apply (ft_t equ_clos_gt); econstructor; [eauto | | symmetry; eauto]; assumption.
Qed.

#[global] Instance equ_clos_gt_ctx E B X RR `(HasB1 : B1 -< B) :
  Proper (@equ E B X X eq ==> equ eq ==> impl) (gt eq RR).
Proof.
  cbn; intros ? ? eq1 ? ? eq2 H.
  apply (ft_t equ_clos_gt); econstructor; [symmetry; eauto | | eauto]; assumption.
Qed.

#[global] Instance equ_gbt_closed_goal E B X `(B1 -< B) {r} :
  Proper (@equ E B X X eq ==> equ eq ==> flip impl) (gbt eq r).
Proof.
  repeat intro.
  apply (fbt_bt equ_clos_gt); econstructor; [eauto | | symmetry; eauto]; assumption.
Qed.

#[global] Instance equ_gbt_closed_ctx E B X `(B1 -< B) {r} :
  Proper (@equ E B X X eq ==> equ eq ==> impl) (gbt eq r).
Proof.
  repeat intro.
  apply (fbt_bt equ_clos_gt); econstructor; [symmetry; eauto | | eauto]; assumption.
Qed.

#[global] Instance equ_clos_gT_goal E B X `(HasB1 : B1 -< B) RR f :
  Proper (@equ E B X X eq ==> equ eq ==> flip impl) (gT eq f RR).
Proof.
  cbn; intros ? ? eq1 ? ? eq2 H.
  apply (fT_T equ_clos_gt); econstructor; [eauto | | symmetry; eauto]; assumption.
Qed.

#[global] Instance equ_clos_gT_ctx E B X `(B1 -< B) RR f :
  Proper (@equ E B X X eq ==> equ eq ==> impl) (gT eq f RR).
Proof.
  cbn; intros ? ? eq1 ? ? eq2 ?.
  apply (fT_T equ_clos_gt); econstructor; [symmetry; eauto | | eauto]; assumption.
Qed.

#[global] Instance equ_clos_equg_goal E B X `(B1 -< B) :
  Proper (@equ E B X X eq ==> equ eq ==> flip impl) (equg eq).
Proof.
  cbn; intros ? ? eq1 ? ? eq2 ?.
  apply (ft_t equ_clos_gt); econstructor; [eauto | | symmetry; eauto]; assumption.
Qed.

#[global] Instance equ_clos_equg_ctx E B X `(B1 -< B) :
  Proper (@equ E B X X eq ==> equ eq ==> impl) (equg eq).
Proof.
  cbn; intros ? ? eq1 ? ? eq2 ?.
  apply (ft_t equ_clos_gt); econstructor; [symmetry; eauto | | eauto]; assumption.
Qed.
...
Section bind.
  ...
  Context {E B: Type -> Type} {X1 X2 Y1 Y2: Type}.
  Context `{HasB1 : B1 -< B}.
...
|*)
  Program Definition bind_ctx_equg SS: mon (rel (ctree E B Y1) (ctree E B Y2)) :=
    {|body := fun R => @bind_ctx E E B B X1 X2 Y1 Y2 (equg SS) (pointwise SS R) |}.
  Next Obligation.
...
Lemma gt_clo_bind (E B: Type -> Type) (X1 X2 Y1 Y2 : Type) `(B1 -< B) :
...
Lemma gt_clo_bind_eq (E B: Type -> Type) (X Y1 Y2 : Type) `(B1 -< B) :
...
#[global] Instance bind_equg_cong_gen :
  forall (E B : Type -> Type) (X Y : Type) `(B1 -< B) (R : rel Y Y) RR,
  Proper (equg (@eq X) ==> pointwise_relation X (gt R RR) ==> gt R RR) (@bind E B X Y).
Proof.
  repeat red. intros.
...
|*)
Lemma equg_clo_bind (E B: Type -> Type) (X1 X2 Y1 Y2 : Type) `(B1 -< B) :
	forall (t1 : ctree E B X1) (t2 : ctree E B X2) (k1 : X1 -> ctree E B Y1) (k2 : X2 -> ctree E B Y2)
...
Lemma equg_clo_bind_eq (E B: Type -> Type) (X Y1 Y2 : Type) `(B1 -< B) :
	forall (t : ctree E B X) (k1 : X -> ctree E B Y1) (k2 : X -> ctree E B Y2)
    (R : rel Y1 Y2),
...
Ltac __upto_bind_equg' SS :=
  match goal with
    ...
    |- @equg ?E ?B ?R1 ?R2 _ ?RR (CTree.bind (T := ?T1) _ _) (CTree.bind (T := ?T2) _ _) =>
      apply (@equg_clo_bind E B T1 T2 R1 R2 _ _ _ _ _ SS RR)

    (* Upto when unguarded *)
  | |- body (t (@fequg ?E ?B ?R1 ?R2 _ ?RR)) ?R (CTree.bind (T := ?T1) _ _) (CTree.bind (T := ?T2) _ _) =>
        apply (ft_t (@bind_ctx_equg_t E B T1 T2 R1 R2 _ SS RR)), in_bind_ctx

    (* Upto when guarded *)
  | |- body (bt (@fequg ?E ?B ?R1 ?R2 _ ?RR)) ?R (CTree.bind (T := ?T1) _ _) (CTree.bind (T := ?T2) _ _) =>
      apply (fbt_bt (@bind_ctx_equg_t E B T1 T2 R1 R2 _ SS RR)), in_bind_ctx
  end.
...
Section UpToGuard.
  ...
  Program Definition utg {E B X} `{B1 -< B} : mon (relation (ctree E B X)) := {| body :=
    fun R x y =>
      R x y \/
